...
export default function ConsentPage() {
  ...
  async function toggleConsent(id: string, current: string) {
    ...
    await writeAuditLog(
      'UPDATE',
      userEmail,
      `Consent toggled: ${agreement} → ${newStatus}`
    )

    setConsents(prev =>
      prev.map(c => c.consent_id === id ? { ...c, consent_status: newStatus } : c)
    )
    setUpdatingId(null)
  }
  ...
  async function revokeConsent(id: string) {
    ...
    await writeAuditLog(
      'REVOKE',
      userEmail,
      `Consent revoked: ${agreement} → Opt-out`
    )

    setConsents(prev =>
      prev.map(c => c.consent_id === id ? { ...c, consent_status: 'Opt-out' } : c)
    )
    setUpdatingId(null)
  }
  ...
  async function addConsent() {
    ...
    await supabase.from('Consent_Record').insert({
      user_id: userId, agreement_id: agreementId, consent_status: status
    })
    ...
    await writeAuditLog(
      ...
      `Consent added: ${status} for ${selectedAgreement?.agreement_name || 'Unknown'} by ${selectedUser?.name || 'Unknown'}`
    )

    setUserId(''); setAgreementId(''); setStatus('Opt-in')
    setShowForm(false); fetchConsents(); setAdding(false)
  }
  ...
  async function deleteConsent(id: string) {
    ...
    await writeAuditLog(
      'DELETE',
      userEmail,
      `Consent record deleted: ${agreement} for ${record?.User?.name || 'Unknown'}`
    )
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
              {['#', 'User', 'Agreement', 'Status', 'Date', 'Actions'].map(h => (
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} style={{ padding: '40px', textAlign: 'center', color: '#94a3b8' }}>No consent records yet.</td></tr>
            ) : consents.map((c, i) => (
              <tr key={c.consent_id} style={{ borderBottom: '1px solid #f1f5f9' }}
                ...
                <td style={{ padding: '14px 20px' }}>
                  <div style={{ display: 'flex', gap: '6px' }}>
                    <button
                      onClick={() => toggleConsent(c.consent_id, c.consent_status)}
                    ...
                    <button
                      onClick={() => deleteConsent(c.consent_id)}
